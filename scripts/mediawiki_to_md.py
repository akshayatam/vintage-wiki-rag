...
def clean_filename(title):
    ...
    return INVALID_FILENAME_CHARS.sub('_', title)
...
def extract_image_name(image_value):
    if not image_value:
        return None

    if isinstance(image_value, list):
        joined = " ".join(str(x) for x in image_value).strip()
    else:
        joined = str(image_value).strip()

    match = re.search(r'\[\[(?:File|Image):(.*?)(?:\|.*?)?\]\]', joined, re.IGNORECASE)
    if match:
...
def extract_infobox(wikicode):
    ...
    if raw_name.startswith("infobox_"):
        ...
        infobox_type = str(infobox_template.name).strip()
    ...
    image_value = infobox_data.get("image")
    if image_value:
        image_name = None
        ...
        if match:
            ...
            image_name = joined.strip()

        if image_name:
            local_filename = download_image(image_name)
            if local_filename:
                embed = f"![[{IMAGE_DIR}/{local_filename}]]\n\n"
...
def create_tag_indexes():
    ...
    for tag, pages in tag_to_pages.items():
        ...
        safe_tag_filename = clean_filename(f"_{tag}") + ".md"
        with open(os.path.join(index_dir, safe_tag_filename), "w", encoding="utf-8") as f:
            f.write(content)
